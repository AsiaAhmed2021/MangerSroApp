...
namespace SilkroadSecurityApi
{
  public class Security
  {
    ...
    {
      uint[] securityTable = new uint[65536];
    ...
    {
      byte num = (byte) ((uint) this.m_count_byte_seeds[2] * ((uint) ~this.m_count_byte_seeds[0] + (uint) this.m_count_byte_seeds[1]));
      byte countByte = (byte) ((uint) num ^ (uint) num >> 4);
      if (update)
        this.m_count_byte_seeds[0] = countByte;
      return countByte;
    }
    ...
    public void Recv(TransferBuffer raw_buffer)
    {
      ...
      lock (this.m_class_lock)
      {
        ...
        {
          bool flag = false;
          int num8 = (int) transferBuffer.Buffer[1] << 8 | (int) transferBuffer.Buffer[0];
          if ((num8 & 32768) > 0)
          {
            if (this.m_security_flags.blowfish == (byte) 1)
            {
          ...
          if (flag)
          {
            byte[] src = this.m_blowfish.Decode(transferBuffer.Buffer, 2, transferBuffer.Size - 2);
            byte[] dst = new byte[6 + num8];
            Buffer.BlockCopy((Array) BitConverter.GetBytes((ushort) num8), 0, (Array) dst, 0, 2);
            Buffer.BlockCopy((Array) src, 0, (Array) dst, 2, 4 + num8);
            transferBuffer.Buffer = (byte[]) null;
            transferBuffer.Buffer = dst;
          }
          PacketReader packet_data = new PacketReader(transferBuffer.Buffer);
          int length = (int) packet_data.ReadUInt16();
          ushort num9 = packet_data.ReadUInt16();
          byte num10 = packet_data.ReadByte();
          byte num11 = packet_data.ReadByte();
          if (this.m_client_security && this.m_security_flags.security_bytes == (byte) 1)
          {
            byte countByte = this.GenerateCountByte(true);
            if ((int) num10 != (int) countByte)
              throw new Exception("[SecurityAPI::Recv] Count byte mismatch.");
            if ((flag || this.m_security_flags.security_bytes == (byte) 1 && this.m_security_flags.blowfish == (byte) 0) && (flag || this.m_enc_opcodes.Contains(num9)))
            {
              length |= 32768;
          ...
          if (num9 == (ushort) 20480 || num9 == (ushort) 36864)
          {
            this.Handshake(num9, packet_data, flag);
            Packet packet = new Packet(num9, flag, false, transferBuffer.Buffer, 6, length);
            packet.Lock();
    ...
    {
      List<KeyValuePair<TransferBuffer, Packet>> keyValuePairList = null;
      lock (this.m_class_lock)
      {
