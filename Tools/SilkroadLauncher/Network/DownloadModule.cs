...
namespace SilkroadLauncher.Network
{
    public static class DownloadModule
    {
        ...
        {
            CheckUpdate.Instance.ShowMessage("DownloadModule::Server_Ready");
        ...
        {
            CheckUpdate.Instance.ShowMessage("DownloadModule::Server_FileChunk");

            // Continue adding bytes to the file
            byte[] buffer = e.Packet.GetBytes();
            m_FileStream.Write(buffer, 0, buffer.Length);

            CheckUpdate.Instance.UpdatingBytesDownloading += (uint)buffer.Length;
            // File being downloaded
            CheckUpdate.Instance.UpdatingFilePercentage = (int)(m_FileStream.Length * 100L / DownloadFiles[0].Size);
        }
        public static void Server_FileCompleted(object sender, ClientMsgEventArgs e)
        {
            ...
            CheckUpdate.Instance.ShowMessage($"Server_FileCompleted: {file.ID} - Result:{result}");
            ...
            if (file.ImportToPk2)
            {
                ...
                if (Pk2Writer.Open(pk2Name + ".pk2", LauncherSettings.CLIENT_BLOWFISH_KEY))
                {
                    ...
                    {
                        CheckUpdate.Instance.ShowMessage("Fatal error updating \"" + file.Name + "\" from \"" + pk2Name + "\"!");
                        CheckUpdate.Instance.Exit();
                    }
                    else
                    {
                        CheckUpdate.Instance.ShowMessage($"File {file.Name} imported into the Pk2");
                    }
                ...
                {
                    CheckUpdate.Instance.ShowMessage("Fatal error opening \"" + pk2Name + "\"!");
                    CheckUpdate.Instance.Exit();
                }
                // Delete the file
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        CheckUpdate.Instance.ShowMessage(string.Format(LauncherSettings.MSG_ERR_FILE_UPDATE, file.Name));
                        CheckUpdate.Instance.Exit();
                    };
                }
            }
            ...
            {
                CheckUpdate.Instance.ShowMessage($"Download finished!");

                // Update SV.T on media.pk2
                UpdateSilkroadVersion();

                // Update done!
                CheckUpdate.Instance.IsUpdating = false;
                ...
                else
                {
                    ...
                    CheckUpdate.Instance.Version = DownloadVersion;
                    CheckUpdate.Instance.CanStartGame = true;

                    CheckUpdate.Instance.StartClinet();
        ...
        private static void RequestFileDownload(Client s)
        {
            ...
            response.WriteUInt32(file.ID);
            response.WriteUInt32(0);
            s.Send(response);

            // File being downloaded
            CheckUpdate.Instance.UpdatingFilePath = file.Path + "\\" + file.Name;
            CheckUpdate.Instance.UpdatingFilePercentage = 0;
        }
        /// <summary>
        ...
        private static void UpdateSilkroadVersion()
        {
            ...
            if (Pk2Writer.Open(LauncherSettings.CLIENT_MEDIA_PK2_PATH, LauncherSettings.CLIENT_BLOWFISH_KEY))
            {
                ...
                if (Pk2Writer.ImportFile("SV.T", "Temp\\SV.T"))
                    CheckUpdate.Instance.ShowMessage($"New version created and imported into the Pk2");
                // Close the Pk2
                Pk2Writer.Close();
                // Delete it
